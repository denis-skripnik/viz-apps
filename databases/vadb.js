...
async function updateVa(vote_id, answer_id, login, shares) {
  ...
  try {
      ...
      const db = client.db("viz-votes");

      let collection = db.collection('votesAnswers');

      let res = await collection.updateOne({vote_id, login}, {$set: {vote_id, answer_id, login, shares}}, { upsert: true });
